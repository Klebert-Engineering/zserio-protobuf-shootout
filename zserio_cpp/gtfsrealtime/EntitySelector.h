/**
 * Automatically generated by Zserio C++ extension version 2.9.0.
 * Generator setup: writerCode, pubsubCode, serviceCode, sqlCode, sourcesAmalgamation, stdAllocator.
 */

#ifndef GTFSREALTIME_ENTITY_SELECTOR_H
#define GTFSREALTIME_ENTITY_SELECTOR_H

#include <zserio/Traits.h>
#include <zserio/BitStreamReader.h>
#include <zserio/BitStreamWriter.h>
#include <zserio/AllocatorPropagatingCopy.h>
#include <memory>
#include <zserio/PackingContext.h>
#include <zserio/OptionalHolder.h>
#include <zserio/ArrayTraits.h>
#include <zserio/String.h>
#include <zserio/Types.h>

#include <gtfsrealtime/TripDescriptor.h>

namespace gtfsrealtime
{

class EntitySelector
{
public:
    using allocator_type = ::std::allocator<uint8_t>;

    explicit EntitySelector(const allocator_type& allocator = allocator_type()) noexcept;

    template <typename ZSERIO_T_agency_id,
            typename ZSERIO_T_route_id,
            typename ZSERIO_T_route_type,
            typename ZSERIO_T_trip,
            typename ZSERIO_T_stop_id,
            ::zserio::is_field_constructor_enabled_t<ZSERIO_T_agency_id, EntitySelector, allocator_type> = 0>
    EntitySelector(
            ZSERIO_T_agency_id&& agency_id_,
            ZSERIO_T_route_id&& route_id_,
            ZSERIO_T_route_type&& route_type_,
            ZSERIO_T_trip&& trip_,
            ZSERIO_T_stop_id&& stop_id_,
            const allocator_type& allocator = allocator_type()) :
            EntitySelector(allocator)
    {
        m_agency_id_ = ::std::forward<ZSERIO_T_agency_id>(agency_id_);
        m_route_id_ = ::std::forward<ZSERIO_T_route_id>(route_id_);
        m_route_type_ = ::std::forward<ZSERIO_T_route_type>(route_type_);
        m_trip_ = ::std::forward<ZSERIO_T_trip>(trip_);
        m_stop_id_ = ::std::forward<ZSERIO_T_stop_id>(stop_id_);
    }

    explicit EntitySelector(::zserio::BitStreamReader& in, const allocator_type& allocator = allocator_type());
    explicit EntitySelector(::zserio::PackingContextNode& contextNode,
            ::zserio::BitStreamReader& in, const allocator_type& allocator = allocator_type());

    ~EntitySelector() = default;

    EntitySelector(const EntitySelector&) = default;
    EntitySelector& operator=(const EntitySelector&) = default;

    EntitySelector(EntitySelector&&) = default;
    EntitySelector& operator=(EntitySelector&&) = default;

    EntitySelector(::zserio::PropagateAllocatorT,
            const EntitySelector& other, const allocator_type& allocator);

    const ::zserio::string<>& getAgency_id() const;
    ::zserio::string<>& getAgency_id();
    void setAgency_id(const ::zserio::string<>& agency_id_);
    void setAgency_id(::zserio::string<>&& agency_id_);
    bool isAgency_idUsed() const;
    bool isAgency_idSet() const;
    void resetAgency_id();

    const ::zserio::string<>& getRoute_id() const;
    ::zserio::string<>& getRoute_id();
    void setRoute_id(const ::zserio::string<>& route_id_);
    void setRoute_id(::zserio::string<>&& route_id_);
    bool isRoute_idUsed() const;
    bool isRoute_idSet() const;
    void resetRoute_id();

    int32_t getRoute_type() const;
    void setRoute_type(int32_t route_type_);
    bool isRoute_typeUsed() const;
    bool isRoute_typeSet() const;
    void resetRoute_type();

    const ::gtfsrealtime::TripDescriptor& getTrip() const;
    ::gtfsrealtime::TripDescriptor& getTrip();
    void setTrip(const ::gtfsrealtime::TripDescriptor& trip_);
    void setTrip(::gtfsrealtime::TripDescriptor&& trip_);
    bool isTripUsed() const;
    bool isTripSet() const;
    void resetTrip();

    const ::zserio::string<>& getStop_id() const;
    ::zserio::string<>& getStop_id();
    void setStop_id(const ::zserio::string<>& stop_id_);
    void setStop_id(::zserio::string<>&& stop_id_);
    bool isStop_idUsed() const;
    bool isStop_idSet() const;
    void resetStop_id();

    static void createPackingContext(::zserio::PackingContextNode& contextNode);
    void initPackingContext(::zserio::PackingContextNode& contextNode) const;

    size_t bitSizeOf(size_t bitPosition = 0) const;
    size_t bitSizeOf(::zserio::PackingContextNode& contextNode, size_t bitPosition) const;

    size_t initializeOffsets(size_t bitPosition = 0);
    size_t initializeOffsets(::zserio::PackingContextNode& contextNode, size_t bitPosition);

    bool operator==(const EntitySelector& other) const;
    uint32_t hashCode() const;

    void write(::zserio::BitStreamWriter& out) const;
    void write(::zserio::PackingContextNode& contextNode, ::zserio::BitStreamWriter& out) const;

private:
    ::zserio::InplaceOptionalHolder<::zserio::string<>> readAgency_id(::zserio::BitStreamReader& in,
            const allocator_type& allocator);
    ::zserio::InplaceOptionalHolder<::zserio::string<>> readRoute_id(::zserio::BitStreamReader& in,
            const allocator_type& allocator);
    ::zserio::InplaceOptionalHolder<int32_t> readRoute_type(::zserio::BitStreamReader& in);
    ::zserio::InplaceOptionalHolder<int32_t> readRoute_type(::zserio::PackingContextNode& contextNode,
            ::zserio::BitStreamReader& in);
    ::zserio::InplaceOptionalHolder<::gtfsrealtime::TripDescriptor> readTrip(::zserio::BitStreamReader& in,
            const allocator_type& allocator);
    ::zserio::InplaceOptionalHolder<::gtfsrealtime::TripDescriptor> readTrip(::zserio::PackingContextNode& contextNode,
            ::zserio::BitStreamReader& in, const allocator_type& allocator);
    ::zserio::InplaceOptionalHolder<::zserio::string<>> readStop_id(::zserio::BitStreamReader& in,
            const allocator_type& allocator);

    ::zserio::InplaceOptionalHolder<::zserio::string<>> m_agency_id_;
    ::zserio::InplaceOptionalHolder<::zserio::string<>> m_route_id_;
    ::zserio::InplaceOptionalHolder<int32_t> m_route_type_;
    ::zserio::InplaceOptionalHolder<::gtfsrealtime::TripDescriptor> m_trip_;
    ::zserio::InplaceOptionalHolder<::zserio::string<>> m_stop_id_;
};

} // namespace gtfsrealtime

#endif // GTFSREALTIME_ENTITY_SELECTOR_H
